...
#ifdef NEW
struct HashMap{
    using dtable = upcxx::dist_object<std::unordered_map<std::string, kmer_pair>>;
...
std::vector<FB> HashMap::local_find_many(
    ...
    for(int i = 0; i < n; ++i){
        ...
        kmer_pair &kp = (*table)[key_kmer.get()];
        ret[i].b = kp.fb_ext[0];
        ret[i].f = kp.fb_ext[1];
    }
    return ret;
