...
__global__ void compute_forces_gpu(
    ...
    int bi = bidx / dim;
    int bj = bidx % dim;
    int ni = bi + dir[direction+4][0];
    int nj = bj + dir[direction+4][1];
    ...
    for(int i = bin_start[bidx]; i < bin_start[bidx+1]; ++i){
        ...
        for(int j = (nidx != bidx)? (bin_start[nidx]):(i+1); j < bin_start[nidx+1]; ++j){
...
__global__ void count_each_bin_gpu(
    ...
    /* int bidx = (int)(parts[idx].x / BINSIZE) * dim + (int)(parts[idx].y / BINSIZE); */
...
void simulate_one_step(particle_t* parts, int num_parts, double size) {
    ...
    // Parallel 5 directions
    compute_forces_gpu<<<(5 * num_bins + NUM_THREADS - 1) / NUM_THREADS, NUM_THREADS>>>(
