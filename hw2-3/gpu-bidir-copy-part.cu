...
__device__ void apply_force_bidir_gpu(Particle_w_idx& particle, Particle_w_idx& neighbor) {
...
__global__ void clear_axay(Particle_w_idx *parts, int num_parts){
...
__global__ void compute_forces_gpu(
    ...
    for(int i = bin_start[bidx]; i < bin_start[bidx+1]; ++i){
        Particle_w_idx &p1 = bins[i];
        for(int j = i+1; j < bin_start[bidx+1]; ++j){
            Particle_w_idx &p2 = bins[j];
...
__global__ void move_gpu(Particle_w_idx* particles, int num_parts, double size) {
    ...
    Particle_w_idx* p = &particles[idx];
    //
...
__global__ void put_particle_indices_in_bin_gpu(
        particle_t *parts, Particle_w_idx *bins, int num_parts,
    ...
    // Put this particle in this bin
    /* bins[dst_idx] = parts[idx]; */
    bins[dst_idx] = {.idx=idx,
        .x=parts[idx].x,
        .y=parts[idx].y,
        .vx=parts[idx].vx,
        .vy=parts[idx].vy,
...
__global__ void put_back(particle_t *parts, Particle_w_idx *bins, int num_parts){
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    if(idx >= num_parts) return;
    int dst_idx = bins[idx].idx;
    parts[dst_idx] = {
        .x=bins[idx].x,
        .y=bins[idx].y,
        .vx=bins[idx].vx,
...
void init_simulation(particle_t* parts, int num_parts, double size) {
    ...
    cudaMalloc((void **)&bins, num_parts * sizeof(Particle_w_idx));
...
void simulate_one_step(particle_t* parts, int num_parts, double size) {
    ...
    // Compute (ax, ay)
    clear_axay<<<num_blocks, NUM_THREADS>>>(bins, num_parts);

    // Each thread will be responsible for one bin
    compute_forces_gpu<<<(num_bins + NUM_THREADS - 1) / NUM_THREADS, NUM_THREADS>>>(
            bins, bin_start, num_bins, dim);

    // Each thread move the particle
    move_gpu<<<num_blocks, NUM_THREADS>>>(bins, num_parts, size);

    // Put bins' particles back
    put_back<<<num_blocks, NUM_THREADS>>>(parts, bins, num_parts);
}
