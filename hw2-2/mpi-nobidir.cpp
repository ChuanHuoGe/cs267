...
void init_simulation(particle_t* parts, int num_parts, double size, int rank, int num_procs) {
    ...
    for(auto &kv: temp_rank_grids_send){
        ...
        sendreqs[target_src_rank].resize(grid.size());
        recvreqs[target_src_rank].resize(recv_grid.size());

        rank_grids_send_len[target_src_rank].resize(grid.size());
    }
...
void simulate_one_step(particle_t* parts, int num_parts, double size, int rank, int num_procs) {
    ...
    for(int i = 0; i < num_bins; ++i){
        auto &grid = local_bins[i + (local_start - local_offset)];
...
void gather_for_save(particle_t* parts, int num_parts, double size, int rank, int num_procs) {
    ...
    // Flatten
    for(int i = 0; i < num_bins; ++i){
        int n = local_bins[i + (local_start - local_offset)].size();
        num_parts_to_send += n;
        for(int j = 0; j < n; ++j){
            local_parts_send.push_back(local_bins[i + (local_start - local_offset)][j]);
        }
    }
