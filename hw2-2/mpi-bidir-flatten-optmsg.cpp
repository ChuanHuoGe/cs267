...
// Reduce message sending overhead
struct Particle_xy{
    double x, y;
};
MPI_Datatype PARTICLE_XY;

struct Particle_axay{
    double ax, ay;
};
MPI_Datatype PARTICLE_AXAY;

struct Particle_xyaxay{
    double x, y, ax, ay;
};
...
// bottom right bins
std::vector<std::vector<Particle_xyaxay>> br_local_bins;
...
// send x,y to top left
std::map<Rank, std::vector<Particle_xy>> send_parts_xy;
// recv ax,ay from top left
std::map<Rank, std::vector<Particle_axay>> recv_parts_axay;

// recv x, y from bottom right
std::map<Rank, std::vector<Particle_xy>> recv_parts_xy;
// send ax, ay to bottom right
std::map<Rank, std::vector<Particle_axay>> send_parts_axay;
...
void apply_force_bidir(particle_t& particle, Particle_xyaxay& neighbor) {
    // Calculate Distance
    double dx = neighbor.x - particle.x;
    double dy = neighbor.y - particle.y;
    double r2 = dx * dx + dy * dy;
...
void create_mpi_datatypes(){
    ...
    MPI_Aint offsets[2];
    offsets[0] = offsetof(Particle_xy, x);
    offsets[1] = offsetof(Particle_xy, y);

    MPI_Type_create_struct(nitems, blocklengths, offsets, types, &PARTICLE_XY);
    MPI_Type_commit(&PARTICLE_XY);

    // PARTICLE_AXAY
    offsets[0] = offsetof(Particle_axay, ax);
    offsets[1] = offsetof(Particle_axay, ay);

    MPI_Type_create_struct(nitems, blocklengths, offsets, types, &PARTICLE_AXAY);
    MPI_Type_commit(&PARTICLE_AXAY);
}
...
void init_simulation(particle_t* parts, int num_parts, double size, int rank, int num_procs) {
    ...
    for(auto &kv: temp_rank_grids_send){
        ...
        send_parts_xy[target_rank].reserve(num_parts / num_procs);
        recv_parts_axay[target_rank].reserve(num_parts / num_procs);
    }
    for(auto &kv: temp_rank_grids_recv){
        ...
        // Pre-allocate to save memory reallocation time
        recv_parts_xy[src_rank].reserve(num_parts / num_procs);
        send_parts_axay[src_rank].reserve(num_parts / num_procs);
    }
    ...
    local_bins.resize(num_bins);
    // there will be some redundant spaces, but it will be easier to index
    br_local_bins.resize(num_bins_w_neighbors);
...
void send_particles_xy(int target_rank, int &n,
        std::vector<Particle_xy> &particles,
        std::array<MPI_Request, 2> &reqs){

    MPI_Isend(&n, 1, MPI_INT, target_rank, 0, MPI_COMM_WORLD, &reqs[0]);
    MPI_Isend(particles.data(), n, PARTICLE_XY, target_rank, 0, MPI_COMM_WORLD, &reqs[1]);
}
...
void recv_particles_xy(int src_rank,
        std::vector<Particle_xy> &particles,
        std::array<MPI_Request, 2> &reqs){
    particles.clear();
    int n;
    MPI_Recv(&n, 1, MPI_INT, src_rank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    particles.resize(n);
    MPI_Irecv(particles.data(), n, PARTICLE_XY, src_rank, 0, MPI_COMM_WORLD, &reqs[1]);
}

void send_axay(int target_rank,
        std::vector<Particle_axay> &particles,
        std::array<MPI_Request, 2> &reqs){
    int n = particles.size();
    MPI_Isend(&particles[0], n, PARTICLE_AXAY, target_rank, 0, MPI_COMM_WORLD, &reqs[1]);
}
...
void recv_axay(int src_rank,
    ...
    MPI_Recv(particles.data(), n, PARTICLE_AXAY, src_rank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
void simulate_one_step(particle_t* parts, int num_parts, double size, int rank, int num_procs) {
    ...
    for(auto &kv: rank_grids_send){
        ...
        std::vector<Particle_xy> &parts_to_send = send_parts_xy[target_rank];
        ...
        for(int i = 0; i < n; ++i){
            ...
            for(particle_t &p: grid){
                parts_to_send.push_back({.x=p.x, .y=p.y});
            }
        }
        // Send all particles at once
        send_particles_xy(target_rank, len, parts_to_send, reqs);
    }
    ...
    for(auto &kv: rank_grids_recv){
        ...
        std::vector<Particle_xy> &parts_to_recv = recv_parts_xy[src_rank];
        // Recv all particles at once
        recv_particles_xy(src_rank, parts_to_recv, reqs);
    }
    ...
    for(auto &kv: recvreqs){
        ...
        for(Particle_xy &p: parts_to_recv){
            int bidx = get_global_bin_idx(p.x, p.y);
            // Also clear each particle's acceleration
            br_local_bins[bidx - local_offset].push_back(
                {.x=p.x, .y=p.y, .ax=0, .ay=0});
        }
    }
    ...
    for(int i = 0; i < num_bins; ++i){
        ...
        for(int d = 5; d < 9; ++d){
            ...
            if(get_rank_from_bin_idx(nidx) == rank){
                // if this grid is owned by me
                std::vector<particle_t> &nei_grid = local_bins[nidx - local_offset];
                ...
                std::vector<Particle_xyaxay> &nei_grid = br_local_bins[nidx - local_offset];
                for(particle_t &p1: grid){
                    for(Particle_xyaxay &p2: nei_grid){
                        apply_force_bidir(p1, p2);
                    }
                }
            }
    ...
    for(auto &kv: rank_grids_recv){
        ...
        std::vector<Particle_axay> &parts_to_send = send_parts_axay[target_rank];
        parts_to_send.clear();
        ...
        for(int i = 0; i < n; ++i){
            ...
            auto &grid = br_local_bins[bidx - local_offset];

            // Collect all the particles's (ax, ay)
            for(Particle_xyaxay &p: grid){
                parts_to_send.push_back({.ax=p.ax, .ay=p.ay});
            }
        }
    ...
    for(auto &kv: rank_grids_send){
        ...
        std::vector<Particle_axay> &parts_to_recv = recv_parts_axay[src_rank];

        // # number of particles(x,y) send = # number of particles(ax,ay) receive
        parts_to_recv.resize(send_parts_xy[src_rank].size());
